...
const ZodiacCryptoPredictor: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        // Race the context loading against a timeout.
        // In a real Frame environment, context loads quickly.
        // In a browser/debugging without parent frame, it might hang.
        // We use timeout to unblock the UI and show the Sign In screen instead of mocking data.
        const timeoutPromise = new Promise<null>((resolve) => setTimeout(() => resolve(null), 500));
        const contextPromise = sdk.context;
        
        const context = await Promise.race([contextPromise, timeoutPromise]) as FrameContext | null;
        ...
        // Critical: Always call ready() to signal the Frame is loaded
        sdk.actions.ready();
        setLoading(false);
      }
    };
  ...
  const handleSignIn = useCallback(async () => {
    try {
      ...
      // SignInResult contains signature, message, nonce but not the user object directly.
      // We check if we have a context user available after sign in, or fallback for demo/dev purposes.
      if (result) {
        try {
            // Try to fetch context again in case it resolved after the initial timeout
            const context = await sdk.context;
            if (context && context.user) {
                setUser(context.user as FarcasterUser);
            } else {
                // Fallback mock user for testing/browser environment where context is missing
                setUser({
                    fid: 999999,
  ...
  const downloadPrediction = () => {
    ...
    const content = `
🌌 Crypto Zodiac Prediction for @${username} 🌌
...
Generated via Farcaster Zodiac Crypto Predictor
    ...
    link.download = `crypto-zodiac-${username}-${prediction.sign}.txt`;
    link.href = url;
    link.click();
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-4 w-full sm:w-auto">
              <div className="bg-cyan-500/20 p-3 rounded-xl shrink-0 overflow-hidden">
                {user.pfpUrl ? (
                  <img src={user.pfpUrl} alt={user.username || "User PFP"} className="w-6 h-6 rounded-full object-cover" />
                ) : (
              ...
              <div className="overflow-hidden">
                <h3 className="text-lg sm:text-xl font-bold text-white truncate">
                  {user.displayName || (user.username ? `@${user.username}` : `FID: ${user.fid}`)}
                </h3>
