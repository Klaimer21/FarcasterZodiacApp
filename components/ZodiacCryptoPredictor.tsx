...
const ZodiacCryptoPredictor: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // Call ready() to indicate the frame is loaded and ready to be displayed
        sdk.actions.ready();
      } catch (error) {
  ...
  const handleSignIn = useCallback(async () => {
    try {
      ...
      // In a production app with a backend, this should be generated by the server.
      const nonce = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
      
      const result = await sdk.actions.signIn({ nonce });
      
      // NOTE: In a production app, you would send the signature, message, and nonce 
      // to your backend to validate the session token.
      // Example: 
      ...
      // Attempt to fetch user from context again after sign in
      const context = await sdk.context;
      if (context?.user) {
        setUser(context.user as FarcasterUser);
      }
